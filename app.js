...
renderQuest=function(){renderQuestBeforeAnswerHistory();restoreAndTrackQuestAnswers();const root=document.querySelector('#questContent');if(quests[state.current]?.id===4&&root){root.classList.add('q4-readable',`q4-room-${state.step===0?'prep':state.step===1?'training':'boss'}`)}};

const renderChallengeWithBossGiven=renderChallenge;
renderChallenge=function(q){renderChallengeWithBossGiven(q);const inputs=document.querySelector('.challenge-box .inputs');if(q.id===4){document.querySelector('.content-title').textContent='จงหา B ที่ตั้งฉากกับ A และ C ที่ตั้งฉากกับ -A';document.querySelector('.question')?.remove();document.querySelector('.question-label')?.remove();inputs.insertAdjacentHTML('beforebegin','<div class="q4-boss-given"><small>เวกเตอร์ A ที่โจทย์กำหนดจากข้อ 1</small><strong>A = -20i - 21j</strong><span>A = (-20, -21)</span><div>กลับเครื่องหมายทุกพิกัด → <b>-A = (20, 21)</b></div></div>')}inputs.insertAdjacentHTML('beforebegin','<button class="boss-scratch-btn" id="bossScratchBtn">✎ เปิดกระดาษทด — พิมพ์ วาด และลบได้</button>');document.querySelector('#bossScratchBtn').onclick=()=>document.querySelector('#universalScratch')?.classList.add('open')};
...
function showAdventure(forcedFloor=null){
  ...
  const configs=[
    ...
    {title:'วิหารขนาดเวกเตอร์ · magnitude',item:'ชิ้นส่วนสูตร magnitude',goal:4,top:'#53301e',bottom:'#120b18',accent:'#ffbf4f',tip:'เก็บชิ้นส่วนตามลำดับเพื่อประกอบสูตรหาขนาดของ A⃗ ให้สมบูรณ์',items:[[230,180],[470,470],[760,210],[980,540]],steps:['|A⃗| = √((-20)² + (-21)²)','= √(400 + 441)','= √841','= 29'],foes:[[390,340,1.4,1.7],[700,410,-1.6,1.4],[1000,310,-1.3,-1.7]]},
...
/* A short slogan plus one clear instruction block for floor 4 training. */
const simplifyQ4LessonBeforeSlogan=simplifyQ4Lesson;
simplifyQ4Lesson=function(){
  simplifyQ4LessonBeforeSlogan();
