const quests = [
  ...
  {id:4,name:'แมงมุมฉากศิลา',title:'ผู้ถักใยมุม 90°',glyph:'⊥',quote:'ใยของข้าขวางทุกเส้นทางด้วยมุมฉาก เจ้าจะหาเส้นที่ลอดผ่านได้หรือไม่?',map:'วิหารเวกเตอร์ตั้งฉาก',rune:'รูนแห่งมุมฉาก',intro:'ทางออกถูกผนึกด้วยใยตั้งฉาก ต้องสร้างเวกเตอร์ B ที่ตั้งฉากกับ A และ C ที่ตั้งฉากกับ -A อย่างละหนึ่งตัวอย่าง',learn:{title:'หาเวกเตอร์ตั้งฉากและตรวจด้วย dot product',sample:'กำหนด P = (3, 2) จงหาเวกเตอร์ Q ที่ตั้งฉากกับ P หนึ่งตัวอย่าง',steps:['ถ้าเวกเตอร์เดิมคือ <b>(x, y)</b> เวกเตอร์ตั้งฉากเลือกได้เป็น <b>(y, -x)</b> หรือ <b>(-y, x)</b>','แทน x = 3 และ y = 2 เลือกได้ Q = <b>(2, -3)</b>','ตรวจด้วยสูตร dot product: P · Q = (3)(2) + (2)(-3)','คำนวณได้ 6 - 6 = <b>0</b> ดังนั้น P ⟂ Q และมุมระหว่างกันเท่ากับ 90°'],formula:'P · Q = PₓQₓ + PᵧQᵧ = 0 &nbsp;⇒&nbsp; <em>P ⟂ Q</em>'},hint:'ตัวอย่างง่าย: สำหรับ A = (-20, -21) ใช้ B = (21, -20) และสำหรับ -A = (20, 21) ใช้ C = (-21, 20) กด “เติมตัวอย่าง” ได้เลย',question:'จงหา B ที่ตั้งฉากกับ A และ C ที่ตั้งฉากกับ -A มาอย่างละหนึ่งตัวอย่าง',inputs:[['bx','Bₓ'],['by','Bᵧ'],['cx','Cₓ'],['cy','Cᵧ']],prefill:{bx:21,by:-20,cx:-21,cy:20},validate:v=>{const bx=+v.bx,by=+v.by,cx=+v.cx,cy=+v.cy;return (bx||by)&&(cx||cy)&&-20*bx-21*by===0&&20*cx+21*cy===0},explain:'ผ่านด่าน! ตรวจ B: (-20)(21) + (-21)(-20) = -420 + 420 = 0 และตรวจ C: (20)(-21) + (21)(20) = -420 + 420 = 0 เมื่อ dot product เป็น 0 เวกเตอร์จึงตั้งฉากกัน',vectors:[[ -20,-21,'A','#75859f'],[21,-20,'B','#55e6d9'],[20,21,'-A','#75859f'],[-21,20,'C','#ffd166']]},
...
function finalizeQ4Display(){const details=document.querySelector('.q4-trig'),next=document.querySelector('#nextBtn'),check=document.querySelector('#checkCleanQ4');if(details)details.innerHTML='<summary>เปิดดูตาราง cos ที่ใช้กับ dot product</summary><table class="mini-cos-table"><thead><tr><th>มุม θ</th><th>0°</th><th>90°</th><th>180°</th></tr></thead><tbody><tr><th>cos θ</th><td>1</td><td class="key-cell">0</td><td>-1</td></tr><tr><th>ความหมาย</th><td>ทิศเดียวกัน</td><td class="key-cell">ตั้งฉาก</td><td>ทิศตรงข้าม</td></tr></tbody></table>';if(next){next.disabled=false;next.innerHTML='พร้อมสู้บอส <span>⚔</span>'}if(check)check.addEventListener('click',()=>{const bx=+document.querySelector('#cleanBx').value,by=+document.querySelector('#cleanBy').value,cx=+document.querySelector('#cleanCx').value,cy=+document.querySelector('#cleanCy').value,out=document.querySelector('#cleanQ4Result'),ok=bx===-3&&by===-4&&cx===3&&cy===4;out.className=ok?'practice-ok':'practice-bad';out.textContent=ok?'✓ ถูกต้อง! แสดงกราฟ A, -A, B และ C ด้านล่างแล้ว':'ยังไม่ถูก คำตอบตัวอย่างคือ B = (-3, -4) และ C = (3, 4)';if(ok)drawCleanQ4Graph()})}
...
function checkAnswer(q){const values={};q.inputs.forEach(([k])=>values[k]=document.querySelector(`#${k}`).value.trim());const f=document.querySelector('#feedback');if(Object.values(values).some(v=>v==='')){f.className='feedback bad show';f.textContent='ยังใส่คำตอบไม่ครบ ลองเติมทุกช่องก่อนนะ';beep(false);return}if(q.validate(values)){if(q.id===4){state.b={x:+values.bx,y:+values.by};state.c={x:+values.cx,y:+values.cy};quests[4].explain=`ชัยชนะ! จากคำตอบของเจ้า B = (${state.b.x}, ${state.b.y}) และ C = (${state.c.x}, ${state.c.y}) เมื่อทำให้ยาว 1 แล้ว dot product เท่ากับ ${expectedFinalDot().toFixed(0)} เพราะเวกเตอร์ตั้งฉากกับเส้นเดียวกันจึงชี้ไปทางเดียวกันหรือตรงข้ามกัน`}
...
const vectoriaAudio={ctx:null,master:null,music:null,sfx:null,timer:null,beat:0,bgm:null,boom:null,gateAccepted:false,
  ...
  effect(kind){if(kind==='explode'&&state.sound){if(!this.boom){this.boom=new Audio('boss-explosion.mp3?v=1');this.boom.preload='auto'}this.boom.currentTime=0;this.boom.volume=.92;this.boom.play().catch(()=>{});return}const notes={click:[[420,.06,.055]],good:[[523,.11,.11],[659,.13,.1,.08],[784,.22,.1,.17]],bad:[[220,.14,.1],[165,.24,.09,.11]],hurt:[[880,.09,.18],[660,.1,.18,.07],[390,.2,.16,.15]],collect:[[740,.07,.12],[988,.1,.13,.055],[1318,.18,.12,.13]],monster:[[150,.08,.2],[105,.16,.18,.045],[72,.26,.16,.12]],item:[[659,.08,.08],[880,.12,.09,.07],[1047,.18,.08,.15]],gate:[[196,.2,.08],[294,.25,.09,.12],[392,.35,.1,.25]],portal:[[220,.13,.13],[330,.16,.14,.08],[494,.2,.14,.17],[740,.34,.12,.28]],boss:[[131,.18,.12],[196,.22,.1,.12],[262,.35,.1,.25]]}[kind]||[];notes.forEach(n=>this.tone(n[0],n[1],n[2],kind==='hurt'||kind==='monster'?'sawtooth':'triangle',n[3]||0))},
