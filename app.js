const quests = [
  ...
  {id:4,name:'แมงมุมฉากศิลา',title:'ผู้ถักใยมุม 90°',glyph:'⊥',quote:'ใยของข้าขวางทุกเส้นทางด้วยมุมฉาก เจ้าจะหาเส้นที่ลอดผ่านได้หรือไม่?',map:'วิหารเวกเตอร์ตั้งฉาก',rune:'รูนแห่งมุมฉาก',intro:'ทางออกถูกผนึกด้วยใยตั้งฉาก ด่านนี้ใช้ชุดตัวอย่างมาตรฐานจาก PDF เพื่อให้คำตอบต่อเนื่องไปยังข้อ 5',learn:{title:'หาเวกเตอร์ตั้งฉากและตรวจด้วย dot product',sample:'กำหนด P = (3, 2) จงหาเวกเตอร์ Q ที่ตั้งฉากกับ P หนึ่งตัวอย่าง',steps:['ถ้าเวกเตอร์เดิมคือ <b>(x, y)</b> เวกเตอร์ตั้งฉากเลือกได้เป็น <b>(y, -x)</b> หรือ <b>(-y, x)</b>','แทน x = 3 และ y = 2 เลือกได้ Q = <b>(2, -3)</b>','ตรวจด้วยสูตร dot product: P · Q = (3)(2) + (2)(-3)','คำนวณได้ 6 - 6 = <b>0</b> ดังนั้น P ⟂ Q และมุมระหว่างกันเท่ากับ 90°'],formula:'P · Q = PₓQₓ + PᵧQᵧ = 0 &nbsp;⇒&nbsp; <em>P ⟂ Q</em>'},hint:'ใช้ชุดตัวอย่างมาตรฐานจาก PDF: B = (-21, 20) และ C = (21, -20) ตรวจได้ A · B = 0 และ (-A) · C = 0 และคู่นี้จะใช้ต่อในข้อ 5',question:'จงหาเวกเตอร์ B ที่ตั้งฉาก (perpendicular) กับ A และเวกเตอร์ C ที่ตั้งฉากกับ -A มาอย่างละหนึ่งตัวอย่าง พร้อมวาดลงบนระนาบ xy',inputs:[['bx','Bₓ'],['by','Bᵧ'],['cx','Cₓ'],['cy','Cᵧ']],validate:v=>+v.bx===-21&&+v.by===20&&+v.cx===21&&+v.cy===-20,explain:'ผ่านด่าน! ใช้ชุดมาตรฐานจาก PDF: B = (-21, 20) และ C = (21, -20) ตรวจได้ A · B = (-20)(-21) + (-21)(20) = 0 และ (-A) · C = (20)(21) + (21)(-20) = 0 จึงตั้งฉากกันทั้งสองคู่ และใช้คู่นี้ต่อในข้อ 5',vectors:[[ -20,-21,'A','#75859f'],[-21,20,'B','#55e6d9'],[20,21,'-A','#75859f'],[21,-20,'C','#ffd166']]},
  {id:5,name:'จักรพรรดิยูนิต ผู้คุมขนาดหนึ่ง',title:'ศึกพิชิตเวกเตอร์หนึ่งหน่วย',glyph:'V̂',quote:'เวกเตอร์ทุกตัวต้องผ่านการย่อขนาดให้เหลือหนึ่ง ก่อนจะเปิดเผยทิศทางที่แท้จริง!',map:'ปราสาท Unit Vector',rune:'รูนแห่งเวกเตอร์หนึ่งหน่วย',intro:'จักรพรรดิยูนิตผนึกทางออกไว้ด้วยกฎขนาดหนึ่ง เจ้าต้องทำชุดตัวอย่างมาตรฐานจาก PDF ให้เป็นเวกเตอร์หนึ่งหน่วย แล้วใช้ dot product ตัดสินความสัมพันธ์ของทิศทาง',learn:{title:'Unit vector คือเวกเตอร์ที่ย่อให้ยาว 1',sample:'ให้ P = (3, 4) และ Q = (-3, -4) หา p̂ · q̂',steps:['|P| = |Q| = 5','หารแต่ละองค์ประกอบด้วยขนาด: p̂ = (3/5, 4/5)','q̂ = (-3/5, -4/5)','dot product = (3/5)(-3/5) + (4/5)(-4/5) = -25/25 = <b>-1</b>'],formula:'p̂ · q̂ = cos(θ) · ตรงข้ามกัน θ=180° จึงได้ <em>-1</em>'},hint:'ข้อ 5 ใช้ชุดตัวอย่างมาตรฐานจาก PDF: B = (-21, 20) และ C = (21, -20) โดย C = -B หลังทำเป็น unit vector ค่า dot product จึงเท่ากับ cos(180°) = -1',question:'จงคำนวณ dot product ระหว่าง unit vector ของ B กับ unit vector ของ C',inputs:[['dot','B̂ · Ĉ']],validate:v=>Number.isFinite(Number(v.dot))&&Math.abs(Number(v.dot)+1)<.001,explain:'ชัยชนะ! เมื่อใช้ B = (-21, 20) และ C = (21, -20) จะได้ |B| = |C| = 29, B̂ = (-21/29, 20/29), Ĉ = (21/29, -20/29) และ B̂ · Ĉ = (-441 - 400)/841 = -1 จึงชี้ตรงข้ามกัน',vectors:[[-21/29,20/29,'B̂','#55e6d9'],[21/29,-20/29,'Ĉ','#ffd166']]}
];

const GAME_BUILD_VERSION='2026-08-17-q4-q5-vector-labels';
const defaultState={completed:[],hp:5,xp:0,current:0,step:0,b:{x:-21,y:20},c:{x:21,y:-20},sound:true,adventure:{},answerHistory:{},started:false,buildVersion:GAME_BUILD_VERSION};
let state=loadState(); let selectedChoice=null;
function loadState(){try{const saved=JSON.parse(localStorage.getItem('vectoria-state')||'{}');return saved.buildVersion===GAME_BUILD_VERSION?{...defaultState,...saved}:{...defaultState}}catch{return {...defaultState}}}
...
function checkAnswer(q){const values={};q.inputs.forEach(([k])=>values[k]=document.querySelector(`#${k}`).value.trim());const f=document.querySelector('#feedback');if(Object.values(values).some(v=>v==='')){f.className='feedback bad show';f.textContent='ยังใส่คำตอบไม่ครบ ลองเติมทุกช่องก่อนนะ';beep(false);return}if(q.validate(values)){if(q.id===4){state.b={x:-21,y:20};state.c={x:21,y:-20}}
...
function restoreAndTrackQuestAnswers(){
  ...
  const q4ManualFields=new Set(['cleanBx','cleanBy','cleanCx','cleanCy','extraBx','extraBy','extraCx','extraCy','bx','by','cx','cy']);
  fields().forEach(el=>{const key=el.id||el.name;if(q.id===4&&q4ManualFields.has(key))return;if(Object.prototype.hasOwnProperty.call(saved,key)){if(el.type==='checkbox'||el.type==='radio')el.checked=Boolean(saved[key]);else el.value=saved[key]}});
...
function showAdventure(forcedFloor=null){
  ...
  function draw(){ctx.drawImage(gameBackground,0,0);
    ...
    if(floor===3){ctx.save();ctx.fillStyle='rgba(3,25,34,.9)';ctx.strokeStyle='#68f0c2';ctx.lineWidth=3;ctx.beginPath();ctx.roundRect(30,70,490,88,15);ctx.fill();ctx.stroke();ctx.fillStyle='#dffff4';ctx.font='700 22px Chakra Petch';ctx.textAlign='left';ctx.fillText('ภารกิจ: ตามหาจุดตัดตั้งฉาก 90°',50,105);ctx.fillStyle='#8ff5d1';ctx.font='600 16px Noto Sans Thai';ctx.fillText('เมื่อ A ⟂ B จะได้ A · B = 0',50,136);cfg.items.forEach((p,i)=>{const px=p[0],py=p[1],len=72,head=22;ctx.shadowBlur=18;ctx.shadowColor='#51e8bb';ctx.strokeStyle=ctx.fillStyle='rgba(78,224,255,.92)';ctx.lineWidth=8;ctx.beginPath();ctx.moveTo(px,py);ctx.lineTo(px+len,py);ctx.stroke();ctx.beginPath();ctx.moveTo(px+len,py);ctx.lineTo(px+len-head,py-head*.55);ctx.lineTo(px+len-head,py+head*.55);ctx.closePath();ctx.fill();ctx.strokeStyle=ctx.fillStyle='rgba(255,211,91,.95)';ctx.beginPath();ctx.moveTo(px,py);ctx.lineTo(px,py-len);ctx.stroke();ctx.beginPath();ctx.moveTo(px,py-len);ctx.lineTo(px-head*.55,py-len+head);ctx.lineTo(px+head*.55,py-len+head);ctx.closePath();ctx.fill();ctx.shadowBlur=0;ctx.strokeStyle='#fff';ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(px+14,py);ctx.lineTo(px+14,py-14);ctx.lineTo(px,py-14);ctx.stroke();ctx.fillStyle='#b8f8ff';ctx.font='800 15px Chakra Petch';ctx.textAlign='center';ctx.fillText('A',px+len+13,py+5);ctx.fillStyle='#ffe58a';ctx.fillText('B',px+5,py-len-10);ctx.fillStyle='#e9fff7';ctx.font='700 13px Chakra Petch';ctx.fillText(`${i+1} · 90°`,px,py+len+23)});ctx.restore()}
